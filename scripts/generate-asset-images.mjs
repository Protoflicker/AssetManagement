// Generate a WebP illustration per asset name (icon matched by keyword + the
// item name), written to public/assets/aset/<slug>.webp. The frontend (db.js)
// falls back to that conventional path whenever an asset has no uploaded photo.
//
//   npm i --no-save xlsx sharp
//   node scripts/generate-asset-images.mjs
//
// Source of names: "daftar-aset 2025-1.xlsx" (kolom "Nama Barang").
...
for (const [key, name] of names) {
  ...
  await sharp(Buffer.from(svgCard(name))).webp({ quality: 82 }).toFile(out);
  done++;
}
console.log('OK: ' + done + ' gambar webp dibuat di public/assets/aset/');
