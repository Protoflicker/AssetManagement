...
(function () {
  ...
  function injectToggleButton() {
    if (document.getElementById('sesd-theme-toggle')) return;

    var spans = document.querySelectorAll('span');
    var onlineSpan = null;
    for (var i = 0; i < spans.length; i++) {
      if (spans[i].textContent.trim() === 'Online') {
        onlineSpan = spans[i];
        break;
      }
    }
    ...
    var onlineContainer = onlineSpan.parentElement;

    var label = document.createElement('label');
    label.className = 'theme-switch';
    label.id = 'sesd-theme-toggle';
    // Add margin if needed to separate it a bit from the date text if any
    label.style.marginLeft = '4px';

    var html = '<input type="checkbox" class="theme-switch__checkbox" id="theme-switch-checkbox">';
    ...
    onlineContainer.parentElement.insertBefore(label, onlineContainer.nextSibling);
  }
